 ...
 *
 * mu͒2 note: measure noOffset and irregular properties aren’t exposed
 * to QML, so we’re counting measures one by one here (instead of the
 * proper measure number). Port to mu͒2 done for Jinx from IRC ☺
 */
	...
	function buildMeasureMap(score) {
		...
		while (cursor.measure) {
			...
			while (s !== null) {
				...
				console.log("TimeSig found in m." + no);
				var e = s.elementAt(0);
				if (e === null) {
					console.log("TimeSig was nil");
					thisTS = null;
					break;
				}
				thisTS = [e.denominator, e.numerator];
				break;
			}
			if (thisTS !== null)
				lastTS = thisTS;
